  ...
  if (collapsed) {
    return (
      <div className="w-12 bg-card border-r border-border flex flex-col items-center py-4">
        ...
        <div className="mt-4 text-xs font-medium text-muted-foreground writing-vertical">Jenis Cumi</div>
      </div>
    );
  }
            ...
            {recentItems.map((item) => (
              <button key={item} type="button" onClick={() => onSelect(item)}
                className={cn("px-3 py-1.5 rounded-lg text-sm font-bold transition-all",
                  selectedJenis === item ? "bg-accent text-accent-foreground" : "bg-muted hover:bg-muted/80 text-foreground")}>
                {item}
              </button>
            ))}
          </div>
      ...
      <div className="p-3 border-b border-border">
        <div className="relative">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Cari jenis cumi..." className="pl-9 pr-9 h-10 text-sm" />
          {search && (
              ...
              return (
                <button key={item} type="button" onClick={() => onSelect(item)}
                  className={cn("w-full flex items-center justify-between p-3 rounded-lg text-left transition-all",
                    selectedJenis === item ? "bg-accent text-accent-foreground font-bold"
                      : isWeighed ? "bg-primary/15 text-primary font-bold border border-primary/30"
