...
const Dashboard = () => {
  ...
  const weightByCategory = useMemo(() => {
    const cats: Record<string, number> = { Ikan: 0, 'Cumi-Cumi': 0, Sotong: 0, Gurita: 0 };
  ...
  const handleDownloadMonthlyReport = () => {
    ...
    monthKapal.forEach((k, idx) => {
      ...
      let ikan = 0, cumiCumi = 0, sotong = 0, gurita = 0;
      k.entries.forEach(e => {
        if (k.jenisPendataan === 'ikan') ikan += e.berat;
        else {
          const cat = getCumiCategory(e.jenis);
          if (cat === 'Cumi-Cumi') cumiCumi += e.berat;
          else if (cat === 'Sotong') sotong += e.berat;
          else gurita += e.berat;
        }
      });
    ...
    const totals = { ikan: 0, cumiCumi: 0, sotong: 0, gurita: 0, all: 0 };
    monthKapal.forEach(k => {
      k.entries.forEach(e => {
        totals.all += e.berat;
        if (k.jenisPendataan === 'ikan') totals.ikan += e.berat;
          ...
          else if (cat === 'Sotong') totals.sotong += e.berat;
